...
public class NotesViewController implements IShutdownable {
	...
	public void shutdown() {
		logInfo("Shutting down NotesViewController and all resources...");
		if (codesWindow != null) {
			codesWindow.closeWindow();
		}
		if (notesViewController == this) {
			notesViewController = null;
			codesWindow = null;
	...
	public void initialize() throws IOException {
		...
		codeSelectionlbl
				.setText(localization.getLocalizedMessage("NotesWindow.CodeSelectionHeading", "Code Selection Menu"));
	}
				...
				modeToggle.setOnAction(_ -> {
					try {
						if (ConfigReader.configRead("notepad", "notepadMode").equals("Light")) {
							...
							if (noteArea != null) {
								noteArea.setStyle(
										"-fx-background-color: #666; -fx-text-fill: white; -fx-border-color: transparent;-fx-background-radius: 0; -fx-border-radius: 0;");
							}
							modeToggle.setStyle(
							...
							if (ConfigReader.configRead("notepad", "notepadMode").equals("Dark")) {
								...
								modeToggle.setStyle("-fx-background-color: grey;");
								notesViewController.getCodeSelectionPane()
										.setStyle("-fx-background-color: rgb(240,240,240,0.1);");
								notesViewController.getCodevbox()
										.setStyle("-fx-background-color: rgb(210,210,210,0.3);");
