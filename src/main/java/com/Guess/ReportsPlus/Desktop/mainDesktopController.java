...
public class mainDesktopController {
			...
			try {
				if (ConfigReader.configRead("desktopSettings", "useBackground").equalsIgnoreCase("true")) {
					...
					if (path != null && !path.isEmpty()) {
						...
						BackgroundImage errorBkgImg = new BackgroundImage(errorImg, BackgroundRepeat.NO_REPEAT, BackgroundRepeat.NO_REPEAT, BackgroundPosition.CENTER,
						                                                  new BackgroundSize(BackgroundSize.AUTO, BackgroundSize.AUTO, true, true, false, false));
	...
	private ContextMenu createReportMenu() {
		...
		reportContextMenu.getItems().addAll(accident, arrest, callout, death, impound, incident, patrol, search, trafficCitation, trafficStop, testBtn);
	...
	public void initialize() throws IOException {
		...
		Timeline clock = new Timeline(new KeyFrame(Duration.ZERO, e -> updateTime()), new KeyFrame(Duration.seconds(1)));
	...
	private void addApps() {
		...
		VBox settingsApp = settingsAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow settingsWindow = WindowManager.createCustomWindow(desktopContainer, "Windows/Settings/settings-view.fxml", "Program Settings", true, 1, true, false, taskBarApps,
		...
		VBox logBrowserApp = logBrowserAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow logapp = WindowManager.createCustomWindow(desktopContainer, "Windows/Apps/log-view.fxml", "Log Viewer", true, 1, true, false, taskBarApps,
		...
		VBox calloutManagerApp = calloutManagerAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow logapp = WindowManager.createCustomWindow(desktopContainer, "Windows/Apps/callout-view.fxml", "Callout Manager", true, 1, true, false, taskBarApps,
		...
		VBox courtApp = courtAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow logapp = WindowManager.createCustomWindow(desktopContainer, "Windows/Apps/court-view.fxml", "Court Case Manager", true, 1, true, false, taskBarApps,
		...
		VBox lookupApp = pedLookupAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow logapp = WindowManager.createCustomWindow(desktopContainer, "Windows/Apps/lookup-ped-view.fxml", "Pedestrian Lookup", true, 1, true, false, taskBarApps,
		...
		VBox vehLookupApp = vehLookupAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow logapp = WindowManager.createCustomWindow(desktopContainer, "Windows/Apps/lookup-veh-view.fxml", "Vehicle Lookup", true, 1, true, false, taskBarApps,
		...
		VBox connectionApp = connectionAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow serverApp = WindowManager.createCustomWindow(desktopContainer, "Windows/Server/client-view.fxml", "Server Connection", false, 1, true, false, taskBarApps,
		...
		VBox showIDApp = showIDAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow IDApp = WindowManager.createCustomWindow(desktopContainer, "Windows/Server/currentID-view.fxml", "Current IDs", false, 1, true, true, taskBarApps,
