...
public class CalloutManager {
	public static void updateCallout(String xmlFile, CalloutDisplayItem calloutDisplayItemToUpdate) {
		...
		try {
			...
			if (calloutFoundAndUpdated) {
				...
				logError("Callout with number " + calloutDisplayItemToUpdate.getNumber() + " not found for update in "
						+ xmlFile);
			}
	...
	public static void addCallout(String xmlFile, String number, String type, String description, String message,
			String priority, String street, String area, String county, String startTime, String startDate,
			String status, String messages) {
		Callouts callouts = null;
		try {
			File file = new File(xmlFile);
			if (file.length() != 0) {
	...
	public static void deleteCallout(String xmlFile, String number) {
		...
		try {
			...
			calloutList = calloutList.stream().filter(callout -> !callout.getNumber().equals(number))
			...
			try {
				...
				if (callouts != null && callouts.getCalloutList() != null) {
					for (Callout callout : callouts.getCalloutList()) {
						displayItems.add(
								new CalloutDisplayItem(callout.getNumber(), callout.getStatus(), callout.getType(),
										callout.getStreet(), callout.getPriority(), callout.getArea(),
										callout.getDescription(), callout.getMessage(), callout.getCounty(),
										callout.getStartDate(), callout.getStartTime(),
										callout.getMessages(), callout));
					}
				}
				tableView.setItems(displayItems);
				if (displayItems.isEmpty()) {
					tableView.setPlaceholder(new Label(localization
							.getLocalizedMessage("Callout_Manager.NoActiveCallouts", "No active callouts.")));
				}
			} catch (JAXBException e) {
				logError("Error loading active callouts: ", e);
				tableView.setPlaceholder(new Label(localization
						.getLocalizedMessage("Callout_Manager.ErrorLoadingActive", "Error loading active callouts.")));
			}
		});
	}
		...
		Platform.runLater(() -> {
			...
			try {
				...
				if (!xmlFile.exists() || xmlFile.length() == 0) {
					tableView.setItems(displayItems);
					tableView.setPlaceholder(new Label(localization
							.getLocalizedMessage("Callout_Manager.NoCalloutHistory", "No callout history.")));
					return;
				}
				JAXBContext jaxbContext = JAXBContext.newInstance(Callouts.class);
				Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();
				...
				tableView.setPlaceholder(new Label(localization
						.getLocalizedMessage("Callout_Manager.ErrorLoadingHistory", "Error loading callout history.")));
			}
		});
	}
