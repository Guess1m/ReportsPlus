...
public class ClientController implements IShutdownable {
	...
	private void addLocalization() {
		...
		portField.setPromptText(
				localization.getLocalizedMessage("ServerConnectionWindow.NotConnected", "Not Connected"));
		inputHostField
				.setPromptText(localization.getLocalizedMessage("ServerConnectionWindow.AddressPrompt", "Address"));
	}
	...
	public void connectBtnPress() throws IOException {
		if (!isConnected) {
			if (!inputHostField.getText().isEmpty() && !inputPortField.getText().isEmpty()) {
				...
				pause.setOnFinished(_ -> statusLabel.setText(beforeText));
				pause.play();
			}
		} else {
	...
	private void showHelpDialog(Stage owner) {
		...
		helpDialog.setContentText(
				localization.getLocalizedMessage("ServerConnectionWindow.HelpLineOne",
