...
import static com.Guess.ReportsPlus.util.Misc.NotificationManager.notificationContainer;
import static com.Guess.ReportsPlus.util.Misc.NotificationManager.notificationContainerScrollPane;
...
import com.Guess.ReportsPlus.Windows.Apps.CalloutViewController;
import com.Guess.ReportsPlus.Windows.Apps.CourtViewController;
import com.Guess.ReportsPlus.Windows.Apps.LogViewController;
import com.Guess.ReportsPlus.Windows.Apps.NewReportVewController;
import com.Guess.ReportsPlus.Windows.Apps.PedLookupViewController;
import com.Guess.ReportsPlus.Windows.Apps.ReportStatisticsController;
...
public class mainDesktopController {
	...
	public void initialize() throws IOException {
		...
		button1.setOnAction(event -> {
			...
			if (!editableDesktop) {
				...
				for (DesktopApp desktopApp : DesktopApps) {
					...
					infoLabelLeft
							.setText(localization.getLocalizedMessage("Desktop.CurrentModeLabel", "Current Mode:"));
		...
		lookupLabel.setOnMouseClicked(mouseEvent -> {
			if (getTopBarHboxRight().getChildren().contains(lookupLabel)) {
				Clipboard clipboard = Clipboard.getSystemClipboard();
				ClipboardContent content = new ClipboardContent();
				String split = lookupLabel.getText()
						.replace(localization.getLocalizedMessage("Desktop.CheckedLabel", "Checked:") + " ", "");
	...
	private void checkForServerSameDir() {
		...
		if (jarFile.exists()) {
			showNotificationErrorPersistent("ERROR",
					"Remove ReportsPlus.jar from the same directory as server jar file");
		} else {
	...
	private void keybindEvent(KeyEvent event) {
		...
		try {
			...
			KeyCode minimizeKey = KeyCode
					.valueOf(ConfigReader.configRead("keybindings", "minimizeWindow").toUpperCase());
			KeyCode maximizeKey = KeyCode
	...
	private void addApps() {
			...
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow mainApp = WindowManager.createCustomWindow(desktopContainer,
							"Windows/Other/notes-view.fxml", "Notes", true, 1, true, false, taskBarApps,
			...
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow settingsWindow = WindowManager.createCustomWindow(desktopContainer,
							"Windows/Settings/settings-view.fxml", "Program Settings", true, 1, true, false,
			...
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					WindowManager.createCustomWindow(desktopContainer, "Windows/Misc/updates-view.fxml",
							"Version Information", true, 1, true, false, taskBarApps, updatesAppObj.getImage());
					try {
						settingsController.loadTheme();
					} catch (IOException e) {
		...
		VBox logBrowserApp = logBrowserAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					logapp = WindowManager.createCustomWindow(desktopContainer, "Windows/Apps/log-view.fxml",
							"Log Viewer", true, 1, true, false, taskBarApps, logBrowserAppObj.getImage());
			...
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow logapp = WindowManager.createCustomWindow(desktopContainer,
							"Windows/Apps/callout-view.fxml", "Callout Manager", true, 1, true, false, taskBarApps,
		...
		VBox courtApp = courtAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow logapp = WindowManager.createCustomWindow(desktopContainer,
							"Windows/Apps/court-view.fxml", "Court Case Manager", true, 1, true, false, taskBarApps,
		...
		VBox lookupApp = pedLookupAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow logapp = WindowManager.createCustomWindow(desktopContainer,
							"Windows/Apps/lookup-ped-view.fxml", "Pedestrian Lookup", true, 1, true, false, taskBarApps,
		...
		VBox vehLookupApp = vehLookupAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow logapp = WindowManager.createCustomWindow(desktopContainer,
							"Windows/Apps/lookup-veh-view.fxml", "Vehicle Lookup", true, 1, true, false, taskBarApps,
			...
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					newReportWindow = WindowManager.createCustomWindow(desktopContainer,
							"Windows/Apps/new-report-view.fxml", "New Report", true, 1, true, false, taskBarApps,
			...
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow serverApp = WindowManager.createCustomWindow(desktopContainer,
							"Windows/Server/client-view.fxml", "Server Connection", false, 1, true, false, taskBarApps,
		...
		VBox showIDApp = showIDAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					WindowManager.createCustomWindow(desktopContainer, "Windows/Server/currentID-view.fxml",
							"Current IDs", true, 1, true, true, taskBarApps, showIDAppObj.getImage());
					try {
						settingsController.loadTheme();
					} catch (IOException e) {
		...
		VBox profileApp = profileAppObj.createDesktopApp(mouseEvent -> {
			if (!editableDesktop) {
				if (mouseEvent.getClickCount() == 2) {
					CustomWindow userManager = WindowManager.createCustomWindow(desktopContainer,
							"Windows/Misc/new-user-manager.fxml", "Profile", true, 1, true, false, taskBarApps,
							profileAppObj.getImage());
					newUserManagerController = (NewUserManagerController) (userManager != null ? userManager.controller
							: null);
					try {
						settingsController.loadTheme();
					} catch (IOException e) {
		...
		if (!isConnected) {
			...
			if (clientController != null) {
				...
				clientController.getStatusLabel()
						.setText(localization.getLocalizedMessage("ServerConnectionWindow.Connected", "Connected"));
