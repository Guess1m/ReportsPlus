...
import static com.Guess.ReportsPlus.Desktop.Utils.WindowUtils.WindowManager.windowPositions;
import static com.Guess.ReportsPlus.Desktop.Utils.WindowUtils.WindowManager.windowSizes;
...
public class CustomWindow {
	...
	// From Code
	public CustomWindow(BorderPane window, String title, boolean resizable, int priority, HBox taskBarApps,
			AnchorPane root, Image image) {
	...
	// From FXML
	public CustomWindow(String fileName, String title, boolean resizable, int priority, HBox taskBarApps,
			AnchorPane root, Image image) {
	...
	private void enableResize(BorderPane pane) {
		...
		pane.setOnMouseDragged(event -> {
			...
			if (pane.getCursor() == javafx.scene.Cursor.SE_RESIZE) {
				...
				double newWidth = Math.min(Math.max(pane.getPrefWidth() - x, 400),
	...
	public void bringToFront() {
		Platform.runLater(() -> {
			...
			windows.values().stream().filter(window -> window.getPriority() > currentPriority)
					.forEach(window -> window.getWindowPane().toFront());
			windows.values().stream().filter(window -> window.getPriority() < currentPriority)
					.forEach(window -> window.getWindowPane().toBack());
			activeWindow = this;
	...
	public void closeWindow() {
		Platform.runLater(() -> {
			...
			if (controller instanceof IShutdownable) {
				try {
					((IShutdownable) controller).shutdown();
				} catch (Exception e) {
					logError("Error during controller shutdown for window: " + title, e);
				}
			}
