...
public class ALPRViewController {
    ...
    public static void loadData() {
        if (alprViewController == null) return;
        File alprFile = new File(serverALPRFileURL);
        if (!alprFile.exists()) return;
        Platform.runLater(() -> {
            try {
    ...
    private Color determineFlagColor(String flag) {
        if (flag.contains("Stolen") || flag.contains("No")) return Color.rgb(255, 104, 99);
        if (flag.contains("Expired")) return Color.rgb(255, 130, 76);
