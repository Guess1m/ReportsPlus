...
public class CalloutViewController {
	...
	private void setCellTextAndPriorityStyle(TableCell<CalloutDisplayItem, String> cell, String textValue, CalloutDisplayItem rowItem) {
		if (textValue == null) {
			cell.setText(null);
			cell.setStyle("");
			return;
		...
		String styleToApply = "-fx-text-fill: " + getTextColorForPriority(null) + ";  -fx-font-family: 'Inter 24pt Regular';";
		
		if (rowItem != null) {
			String priority = rowItem.getPriority();
			styleToApply = "-fx-text-fill: " + getTextColorForPriority(priority) + "; -fx-font-family: 'Inter 24pt Regular';";
		}
	...
	public void initialize() {
		...
		activeCalloutsTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
			if (newSelection != null) {
				showCallDetails(newSelection);
			}
		});
		
		historyCalloutsTable.getSelectionModel().selectedItemProperty().addListener((obs, oldSelection, newSelection) -> {
			if (newSelection != null) {
				showCallDetails(newSelection);
			}
		});
	...
	private void loadLocale() {
		...
		displayActiveCallsBtn.setText(localization.getLocalizedMessage("Callout_Manager.ActiveCallsLabel", "Active Calls"));
		displayCallHistoryBtn.setText(localization.getLocalizedMessage("Callout_Manager.CallHistoryLabel", "Call History"));
	...
	private void refreshNotesForCallout(CalloutDisplayItem callout) {
		...
		if (callout.getDescription() != null && !callout.getDescription().isEmpty()) {
			notesContainer.getChildren().add(createCalloutNote(callout.getStartDate(), callout.getStartTime(), "DISPATCH", callout.getDescription(), null, callout));
		}
		
		if (callout.getMessage() != null && !callout.getMessage().isEmpty()) {
			notesContainer.getChildren().add(createCalloutNote(callout.getStartDate(), callout.getStartTime(), "DISPATCH", callout.getMessage(), null, callout));
		}
		
		for (StoredMessage storedMessage : callout.getStoredMessages()) {
			notesContainer.getChildren().add(createCalloutNote(storedMessage.getDate(), storedMessage.getTime(), storedMessage.getSender(), storedMessage.getMessage(), storedMessage, callout));
		}
	}
	
	private GridPane createCalloutNote(String date, String time, String sender, String text, StoredMessage message, CalloutDisplayItem callout) {
		...
		try {
			if (ConfigReader.configRead("uiColors", "UIDarkMode").equalsIgnoreCase("true")) {
				container.setStyle("-fx-background-color: rgb(0,0,0,0.05); -fx-border-color: " + accclr + "; -fx-border-width: 0 0 1 0;");
				...
				container.setStyle("-fx-background-color: rgb(255,255,255,0.05); -fx-border-color: " + accclr + "; -fx-border-width: 0 0 1 0;");
			...
			senderlbl.setStyle("-fx-text-fill: " + secclr + "; -fx-font-family: 'Inter 28pt Medium'; -fx-font-size: 14;");
		} catch (IOException e) {
	...
	private void setupActiveCalloutsTable() {
		...
		activeCalloutsTable.setPlaceholder(new Label(localization.getLocalizedMessage("Callout_Manager.NoActiveCallouts", "No active callouts.")));
		...
		TableColumn<CalloutDisplayItem, String> numberCol = new TableColumn<>(localization.getLocalizedMessage("Callout_Manager.CalloutNumberShort", "Call#"));
		numberCol.setCellValueFactory(new PropertyValueFactory<>("number"));
		numberCol.setPrefWidth(50);
		numberCol.setCellFactory(priorityStyledCellFactory);
		
		TableColumn<CalloutDisplayItem, String> streetCol = new TableColumn<>(localization.getLocalizedMessage("Callout_Manager.CalloutStreet", "Street"));
		streetCol.setCellValueFactory(new PropertyValueFactory<>("street"));
		streetCol.setPrefWidth(120);
		streetCol.setCellFactory(priorityStyledCellFactory);
		
		TableColumn<CalloutDisplayItem, String> areaCol = new TableColumn<>(localization.getLocalizedMessage("Callout_Manager.CalloutArea", "Area"));
		areaCol.setCellValueFactory(new PropertyValueFactory<>("area"));
		areaCol.setPrefWidth(90);
		areaCol.setCellFactory(priorityStyledCellFactory);
		
		TableColumn<CalloutDisplayItem, String> countyCol = new TableColumn<>(localization.getLocalizedMessage("Callout_Manager.CalloutCounty", "County"));
		...
		TableColumn<CalloutDisplayItem, String> typeCol = new TableColumn<>(localization.getLocalizedMessage("Callout_Manager.CalloutType", "Type"));
		typeCol.setCellValueFactory(new PropertyValueFactory<>("type"));
		typeCol.setPrefWidth(150);
		typeCol.setCellFactory(priorityStyledCellFactory);
		
		TableColumn<CalloutDisplayItem, String> priorityCol = new TableColumn<>(localization.getLocalizedMessage("Callout_Manager.CalloutPriority", "Priority"));
		priorityCol.setCellValueFactory(new PropertyValueFactory<>("priority"));
		priorityCol.setPrefWidth(80);
		priorityCol.setCellFactory(priorityStyledCellFactory);
		
		TableColumn<CalloutDisplayItem, String> statusCol = new TableColumn<>(localization.getLocalizedMessage("Callout_Manager.CalloutStatus", "Status"));
		...
		statusCol.setCellFactory(column -> new TableCell<>() {
				...
				statusDropdown.setOnAction(event -> {
					if (getTableRow() != null && getTableRow().getItem() != null) {
						...
						if (item != null && selectedStatus != null && !selectedStatus.equals(item.getStatus())) {
							...
							CalloutManager.setValueByNumber(URLStrings.calloutDataURL, item.getNumber(), "Status", selectedStatus);
							...
							item.addStoredMessage(new StoredMessage(rank + " " + sender, getDate(), getTime(false, false), "Status", "Status updated to " + selectedStatus + "."));
			...
			private void updateComboBoxStyle(String status) {
				...
				statusDropdown.pseudoClassStateChanged(PseudoClass.getPseudoClass("responded"), status.equals("Responded"));
				statusDropdown.pseudoClassStateChanged(PseudoClass.getPseudoClass("notresponded"), status.equals("Not Responded"));
			}
		...
		TableColumn<CalloutDisplayItem, Void> actionCol = new TableColumn<>(localization.getLocalizedMessage("Callout_Manager.Actions", "Actions"));
		...
		Callback<TableColumn<CalloutDisplayItem, Void>, TableCell<CalloutDisplayItem, Void>> cellFactory = param -> {
			return new TableCell<>() {
				private final Button btn = new Button(localization.getLocalizedMessage("Callout_Manager.CloseCalloutButton", "Close Callout"));
				
				{
					try {
						String defStyle = "-fx-background-color: " + controllerUtils.hexToRgba(ConfigReader.configRead("uiColors", "secondaryColor"), 0.5) + "; -fx-border-color: rgb(100,100,100,0.1); -fx-text-fill: white;  -fx-font-family: 'Inter 24pt Regular'; -fx-padding: 3 10 3 10;";
					...
					btn.setOnAction((ActionEvent event) -> {
						...
						if (item != null) {
							CalloutManager.addCallout(calloutHistoryURL, item.getNumber(), item.getType(), item.getDescription(), item.getMessage(), item.getPriority(), item.getStreet(), item.getArea(), item.getCounty(), item.getStartTime(), item.getStartDate(), item.getStatus(),
							                          item.getMessages());
		...
		activeCalloutsTable.getColumns().addAll(numberCol, streetCol, areaCol, countyCol, typeCol, priorityCol, statusCol, actionCol);
	...
	private void setupHistoryCalloutsTable() {
		historyCalloutsTable.setPlaceholder(new Label(localization.getLocalizedMessage("Callout_Manager.NoCalloutHistory", "No callout history.")));
			...
			protected void updateItem(String statusValue, boolean empty) {
				...
				if (empty || statusValue == null) {
					...
					if ("Not Responded".equals(statusValue)) {
						finalLabelStyle = baseStyle + "-fx-text-fill: white; -fx-background-color: #e53e3e;  -fx-font-family: 'Inter 24pt Regular';";
					} else if ("Responded".equals(statusValue)) {
						finalLabelStyle = baseStyle + "-fx-text-fill: white; -fx-background-color: #38a169;  -fx-font-family: 'Inter 24pt Regular';";
					} else {
						
						finalLabelStyle = baseStyle + "-fx-background-color: #e2e8f0; -fx-text-fill: " + priorityTextColor + "; -fx-font-family: 'Inter 24pt Regular';";
					}
		...
		Callback<TableColumn<CalloutDisplayItem, Void>, TableCell<CalloutDisplayItem, Void>> cellFactory = param -> {
			return new TableCell<>() {
				private final Button createReportBtn = new Button(localization.getLocalizedMessage("Callout_Manager.NewCalloutButtonShort", "Report"));
				private final Button deleteHistoryBtn = new Button(localization.getLocalizedMessage("Callout_Manager.DelCalloutButton", "Delete"));
				private final HBox buttonsPane = new HBox(5);
				
				{
					buttonsPane.setAlignment(Pos.CENTER);
					try {
						String defStyle = "-fx-background-color: " + controllerUtils.hexToRgba(ConfigReader.configRead("uiColors", "secondaryColor"), 0.7) + "; -fx-border-color: rgb(100,100,100,0.1); -fx-text-fill: white; -fx-font-family: 'Inter 24pt Regular'; -fx-padding: 3 10 3 10;";
					...
					createReportBtn.setOnAction((ActionEvent event) -> {
						...
						if (item != null) {
							...
							Map<String, Object> calloutControls = (Map<String, Object>) calloutReportObj.get(localization.getLocalizedMessage("ReportWindows.CalloutReportTitle", "Callout Report") + " Map");
							
							((TextField) calloutControls.get(localization.getLocalizedMessage("ReportWindows.CalloutNumberField", "callout num"))).setText(item.getNumber());
							((ComboBox<String>) calloutControls.get(localization.getLocalizedMessage("ReportWindows.FieldArea", "area"))).setValue(item.getArea());
							String notes = item.getDescription();
							if (item.getMessage() != null && !item.getMessage().isEmpty()) {
								notes += "\n" + item.getMessage();
							}
							((TextArea) calloutControls.get(localization.getLocalizedMessage("ReportWindows.FieldNotes", "notes"))).setText(notes);
							((TextField) calloutControls.get(localization.getLocalizedMessage("ReportWindows.FieldCounty", "county"))).setText(item.getCounty());
							((ComboBox<String>) calloutControls.get(localization.getLocalizedMessage("ReportWindows.FieldStreet", "street"))).setValue(item.getStreet());
							((TextField) calloutControls.get(localization.getLocalizedMessage("ReportWindows.FieldDate", "date"))).setText(item.getStartDate());
							((TextField) calloutControls.get(localization.getLocalizedMessage("ReportWindows.FieldTime", "time"))).setText(item.getStartTime());
							((TextField) calloutControls.get(localization.getLocalizedMessage("ReportWindows.FieldType", "type"))).setText(item.getType());
							((TextField) calloutControls.get(localization.getLocalizedMessage("ReportWindows.CalloutCodeField", "code"))).setText(item.getPriority());
						}
					});
		...
		historyCalloutsTable.getColumns().addAll(numberCol, streetCol, areaCol, countyCol, typeCol, priorityCol, statusCol, actionCol);
	...
	public void createCallBtnPress(ActionEvent actionEvent) {
		...
		try {
			priorityComboBox.getItems().addAll(ConfigReader.configRead("calloutManager", "highPriorityValue"), ConfigReader.configRead("calloutManager", "mediumPriorityValue"), ConfigReader.configRead("calloutManager", "lowPriorityValue"), ConfigReader.configRead("calloutManager", "defaultValue"));
		...
		Label descriptionLabel = new Label(localization.getLocalizedMessage("CalloutPopup.DescriptionLabel", "Description:"));
		...
		Label messageLabel = new Label(localization.getLocalizedMessage("Callout_Manager.Additional", "Additional (Optional):"));
		...
		createButton.setStyle("-fx-background-color: #3f8157; -fx-text-fill: white; -fx-background-radius: 8; -fx-padding: 4 8; -fx-font-family: \"Inter 28pt Medium\"");
		...
		CustomWindow window = createCustomWindow(mainDesktopControllerObj.getDesktopContainer(), createCallWindowRoot, "Create New Callout", true, 1, true, true, mainDesktopControllerObj.getTaskBarApps(),
		...
		createButton.setOnAction(e -> {
			...
			CalloutManager.addCallout(URLStrings.calloutDataURL, number, type, description, message, priority, street, area, county, startTime, localDate, status, null);
	...
	public void settingsBtnPress(ActionEvent actionEvent) {
		...
		saveButton.setStyle("-fx-background-color: #4CAF50; -fx-text-fill: white; -fx-background-radius: 5; -fx-font-family: 'Inter 24pt Regular';");
		...
		saveButton.setOnAction(e -> {
			...
			String p1HexColor = String.format("#%02x%02x%02x", (int) (p1Color.getRed() * 255), (int) (p1Color.getGreen() * 255), (int) (p1Color.getBlue() * 255));
			...
			String p2HexColor = String.format("#%02x%02x%02x", (int) (p2Color.getRed() * 255), (int) (p2Color.getGreen() * 255), (int) (p2Color.getBlue() * 255));
			...
			String p3HexColor = String.format("#%02x%02x%02x", (int) (p3Color.getRed() * 255), (int) (p3Color.getGreen() * 255), (int) (p3Color.getBlue() * 255));
			...
			String p4HexColor = String.format("#%02x%02x%02x", (int) (p4Color.getRed() * 255), (int) (p4Color.getGreen() * 255), (int) (p4Color.getBlue() * 255));
		...
		createCustomWindow(mainDesktopControllerObj.getDesktopContainer(), settingsWindowRoot, "Callout Manager Settings", false, 1, true, true, mainDesktopControllerObj.getTaskBarApps(),
		                   new Image(Objects.requireNonNull(Launcher.class.getResourceAsStream("/com/Guess/ReportsPlus/imgs/icons/Apps/setting.png"))));
	}
	...
	public void onSendNoteClick(ActionEvent actionEvent) {
		...
		selectedCallout.addStoredMessage(new StoredMessage(rank + " " + sender, getDate(), getTime(false, false), "Message", text));
