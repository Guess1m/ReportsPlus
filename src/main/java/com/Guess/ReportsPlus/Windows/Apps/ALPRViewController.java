...
public class ALPRViewController {
	public static class Vehicle {
		...
		public boolean isValid() {
			return licenseplate != null && !licenseplate.isEmpty() && plateType != null && !plateType.isEmpty()
					&& scanner != null && !scanner.isEmpty() && timescanned != null && flags != null;
		}
	}
	...
	private class VehicleListCell extends ListCell<Vehicle> {
		...
		protected void updateItem(Vehicle vehicle, boolean empty) {
			...
			if (empty || vehicle == null) {
				...
				String details = String.format("%.2f mph | %.1f m | %s", vehicle.speed, vehicle.distance,
						vehicle.scanner);
				...
				if (vehicle.flags != null && !vehicle.flags.isEmpty()) {
					vehicle.flags.forEach(flag -> {
						...
						flagLabel.setBackground(new Background(
								new BackgroundFill(determineFlagColor(flag), new CornerRadii(3), Insets.EMPTY)));
		...
		private void setupStyling() {
			plateLabel.setStyle(
					"-fx-text-fill: " + fontcolor + "; -fx-font-family: \"Inter 28pt Bold\"; -fx-font-size: 14px;");
			detailsLabel.setStyle("-fx-text-fill: " + fontcolor
					+ "; -fx-opacity: 0.8; -fx-font-family: \"Inter 24pt Regular\"; -fx-font-size: 12px;");
	...
	private static List<String> parseFlags(String value) {
		...
		return Arrays.stream(value.split("\\R")).map(String::trim).filter(flag -> !flag.isEmpty())
	...
	public void settingsBtnPress(ActionEvent actionEvent) {
		...
		try {
			...
			logError(
					"ALPR; Error reading configValue for useDefaultImage or parsing boolean. Defaulting to use default image.",
					e);
			useDefault = true;
		}
		...
		CustomWindow window = createCustomWindow(mainDesktopControllerObj.getDesktopContainer(), root, "ALPR Settings",
	...
	private void loadLocale() {
		...
		scannedPlatesSubLabel
				.setText(localization.getLocalizedMessage("ALPR.scannedPlatesSubLabel", "Scanned Plates:"));
	...
	private BorderPane Flag(Color color, String flagText) {
		...
		flag.setStyle("-fx-background-color: " + toHexString(color)
				+ "; -fx-background-radius: 7px; -fx-padding: 3px; -fx-opacity: 0.7;");
				...
				if (filePath != null && !filePath.isEmpty()) {
					...
					if (file.exists() && file.isFile()) {
						try {
							...
							logError("ALPR; Failed to load image from existing file [" + filePath
									+ "]. Using default image.", imageLoadEx);
						}
					} else {
						logError("ALPR; Custom image file path [" + (filePath == null ? "null" : filePath)
								+ "] does not exist or is not a file. Using default image.");
					}
				} else {
					logError("ALPR; Custom image file path is null or empty in config. Using default image.");
				}
			} catch (IOException configReadEx) {
				logError("ALPR; IOException while reading 'licensePlateImagePath' from config. Using default image.",
						configReadEx);
			} catch (Exception ex) {
				logError(
